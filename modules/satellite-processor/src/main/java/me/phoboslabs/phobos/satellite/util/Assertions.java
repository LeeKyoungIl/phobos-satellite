...
public final class Assertions {
    ...
    private static final String EXCEPTION_STATUS_MESSAGE_PREFIX = "state should be: ";
    ...
    public static void isTrue(final String name, final boolean condition) {
        if (!condition) {
            throw new IllegalStateException(EXCEPTION_STATUS_MESSAGE_PREFIX + name);
        }
    }
    ...
    public static void isTrueArgument(final String name, final boolean condition) {
        if (!condition) {
            throw new IllegalArgumentException(EXCEPTION_STATUS_MESSAGE_PREFIX + name);
        }
    }

    /**
     * Throw IllegalArgumentException if the condition if false, otherwise return the value.  This is useful when arguments must be checked within an
     * expression, as when using {@code this} to call another constructor, which must be the first line of the calling constructor.
     *
     * @param <T>       the value type
     * @param name      the name of the state that is being checked
